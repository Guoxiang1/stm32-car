...
int mpu_init(void)
{
    ...
    /* Reset device.st.hw->addr = 0x68。AD0 是从 IIC 接口(接 MCU)的地址控制引脚,该引脚控制
IIC 地址的最低位。如果接 GND,则 MPU6050 的 IIC 地址是:0X68,如果接 VDD,则是
0X69。st.reg->pwr_mgmt_1 = 0x6B是MPU6050电源寄存器，对其最高位置1让MPU6050内部所有寄存器恢复默认值。
	复位后,电源管理寄存器 1 恢复默认值(0X40),然后必须设置该寄存器为
...
#if defined MPU6050
    /* Check product revision.accel_offs寄存器 = 0x06,可读取版本号 */
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    /* Set to invalid values to ensure no I2C writes are skipped.  两个传感器的满量程范围(FSR)*/
    st.chip_cfg.sensors = 0xFF;  //使能传感器
    st.chip_cfg.gyro_fsr = 0xFF; //Matches gyro_cfg >> 3 & 0x03
    st.chip_cfg.accel_fsr = 0xFF;//Matches accel_cfg >> 3 & 0x03
    st.chip_cfg.lpf = 0xFF;		//Matches config register
    st.chip_cfg.sample_rate = 0xFFFF; //Sample rate, NOT rate divider(采样频率，不分频)
    st.chip_cfg.fifo_enable = 0xFF; //使能FIFO寄存器
    st.chip_cfg.bypass_mode = 0xFF; //1 if devices on auxiliary I2C bus appear on the primary
#ifdef AK89xx_SECONDARY
    st.chip_cfg.compass_sample_rate = 0xFFFF; //Compass sample rate
#endif
    /* mpu_set_sensors always preserves this setting. */
    st.chip_cfg.clk_src = INV_CLK_PLL; //INV_CLK_PLL = 1 是枚举
    /* Handled in next call to mpu_set_bypass. */
    st.chip_cfg.active_low_int = 1; //1 for active low interrupts
    st.chip_cfg.latched_int = 0; //1 for active low interrupts
    st.chip_cfg.int_motion_only = 0; //1 for active low interrupts
    st.chip_cfg.lp_accel_mode = 0; //1 for active low interrupts
    memset(&st.chip_cfg.cache, 0, sizeof(st.chip_cfg.cache));
    st.chip_cfg.dmp_on = 0;   //1 if DMP is enabled
    st.chip_cfg.dmp_loaded = 0; //Ensures that DMP will only be loaded once
    st.chip_cfg.dmp_sample_rate = 0; //Ensures that DMP will only be loaded once

    if (mpu_set_gyro_fsr(2000)) //设置陀螺仪的满量程范围为±2000dps
        return -1;
    if (mpu_set_accel_fsr(2))   //加速度传感器的满量程范围为±2g
        return -1;
    if (mpu_set_lpf(42))         //数字低通滤波器(DLPF)的设置位 设置加速度传感器和角速度传感器带宽
        return -1;
    if (mpu_set_sample_rate(50)) //陀螺仪采样率通过采样率分频寄存器(0X19)控制,这个采样率我们一般设置为 50 
        return -1;
    if (mpu_configure_fifo(0)) //关闭所有 FIFO 通道,这个通过 FIFO 使能寄存器(0X23)控制,默认都是 0(即禁止 FIFO)
        return -1;
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (sensors & INV_XYZ_GYRO)
        ...
        data = BIT_SLEEP;              /*电源管理寄存器 1 默认值(0X40),然后必须设置该寄存器为0X00,以唤醒 MPU6050,进入正常工作状态*/
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &data)) { //pwr_mgmt_1 = 0x6B器件电源管理寄存器 1， 
